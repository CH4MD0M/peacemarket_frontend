...
import { sendAuthCode, confirmAuthCode } from "@redux/modules/emailSlice";
import { signup } from "@redux/modules/authSlice";
import useInput from "@hooks/useInput";
import AuthTimer from "@util/authTimer";

// CSS
import { Main, Header, Form, Label, Input, Error, LinkContainer, AuthContainer, Section } from "./style";
...
const SignupForm = () => {
  ...
  const { sendAuthCodeState, confirmAuthCodeState, isAutheticated, codeExpirationState } = useSelector(
    (state) => state.email,
  );
  const { fetchSignupState, fetchLoginState, isRegistCompleted } = useSelector((state) => state.auth);
  ...
  // 인증 코드 확인
  const confirmCodeHandler = (e) => {
    e.preventDefault();
    dispatch(confirmAuthCode({ email, signupVerifyToken: authCode }));
  };

  // Form 유효성 검사
  let formIsValid = false;
  if (isAutheticated && nicknameIsValid && passwordIsValid && passwordCheckIsValid) {
    formIsValid = true;
  }
  ...
  useEffect(() => {
    if (isRegistCompleted) navigate("/login", { replace: true });
        ...
        <div>
          ...
          <Form onSubmit={formSubmitHandler}>
            <Label id="email-label" error={emailHasError}>
              ...
              {emailHasError && <Error>Email 형식으로 입력해주세요.</Error>}
              <Button type="button" width="100%" height="4rem" disabled={!emailSendBtnActive} onClick={sendCodeHandler}>
                {sendAuthCodeState === "LOADING" ? <CircularProgress size={20} /> : <p>이메일 인증하기</p>}
              </Button>
              {sendAuthCodeState === "SUCCESS" && (
                <AuthContainer>
                  <span className="auth-message">이메일로 전송된 인증코드를 입력해주세요.</span>
                  <div className="auth-container">
                    <div className="auth-number-container">
                      ...
                      <Button
                        ...
                        {confirmAuthCodeState === "LOADING" ? <CircularProgress size={20} /> : <p>확인</p>}
                      </Button>
                    </div>
                    {codeExpirationState && (
                      <div className="expiration-container">
                        <span className="expiration-message">인증코드가 만료되었습니다.</span>
                        <div>
                          <span>이메일을 받지 못하셨나요?</span>
                          <span onClick={sendCodeHandler}>이메일 재전송하기</span>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <Label id="password-label" error={passwordHasError}>
              ...
              {passwordHasError && <Error>비밀번호 양식에 맞게 입력해주세요.</Error>}
            </Label>
            <Label id="password-check-label" error={passwordCheckHasError}>
              <span>비밀번호 확인</span>
              <div>
                <Input
                  type="password"
              ...
              {passwordCheckHasError && <Error>비밀번호가 일치하지 않습니다.</Error>}
            </Label>
